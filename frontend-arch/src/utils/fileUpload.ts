...
// Fallback simple storage uploader (temporary stub)
export const simpleStorageUpload = async (
  file: File,
  fileType: 'audio' | 'image',
  onProgress?: (p: UploadProgress) => void
): Promise<UploadedFile> => {
  // Simulate 1-second upload delay and progressive callbacks
    ...
    const interval = setInterval(() => {
      ...
      if (loaded >= file.size) {
        ...
        resolve({
          ...
          size: file.size,
          type: file.type,
          url: URL.createObjectURL(file), // local blob URL placeholder
          uploadedAt: new Date()
        });
      }
    }, 100);
